...
class ChaseBallOld(BaseTask):

    def __init__(self, cfg):
        super().__init__(cfg)
        self.num_envs = self.cfg["env"]["num_envs"]
        self.addtional_rigid_num = np.zeros(self.num_envs, dtype=int)
        self._create_envs()
        self.gym.prepare_sim(self.sim)
        self._init_buffers()
        self.current_command = torch.zeros(3, device=self.device)
    ...
    def _create_envs(self):
        ...
        booster_start_pose.p = gymapi.Vec3(*self.base_init_state[:3])
        ...
        for i in range(self.num_envs):
            ...
            booster_start_pose.p = gymapi.Vec3(*pos)
            
            actor_handle = self.gym.create_actor(env_handle, robot_asset, booster_start_pose, booster_asset_cfg["name"], i, booster_asset_cfg["self_collisions"], 0)
            
            ball_offset_x = np.random.uniform(-1,1)
            ball_offset_y = np.random.uniform(-1,1)
            
            ball_pose = gymapi.Transform()
            ball_pose.p = gymapi.Vec3(booster_start_pose.p.x + ball_offset_x, booster_start_pose.p.y + ball_offset_y, ball_radius + 0.01)
            ball_handle = self.gym.create_actor(env_handle, ball_asset, ball_pose, "SoccerBall",i,0)

            self.addtional_rigid_num[i] += 1
            # self.addtional_rigid_num[i] += create_strip_grass(self,env_handle, length=40.0, width=25.0, num_strips=15)
            # self.addtional_rigid_num[i] += create_field_boundary_lines(self,env_handle, length=40.0, width=25.0, line_width=0.15)
            # self.addtional_rigid_num[i] += create_field_auxiliary_lines(self, env_handle, length=40, width=25)
    ...
    def _init_buffers(self):
        ...
        self.root_states_robot = self.root_states[0::2,:]  
        self.root_states_ball = self.root_states[1::2,:]
        self.dof_state = gymtorch.wrap_tensor(dof_state_tensor)
        self.dof_pos = self.dof_state.view(self.num_envs, self.num_dofs, 2)[..., 0]
        self.dof_vel = self.dof_state.view(self.num_envs, self.num_dofs, 2)[..., 1]
        self.contact_forces = gymtorch.wrap_tensor(net_contact_forces).view(self.num_envs, -1, 3)  # shape: num_envs, num_bodies, xyz axis
        self.body_states = gymtorch.wrap_tensor(body_state).view(self.num_envs, self.num_bodies_robot + self.addtional_rigid_num[0], 13)
        ...
        self.pushing_forces = torch.zeros(self.num_envs, self.num_bodies_robot, 3, dtype=torch.float, device=self.device)
        self.pushing_torques = torch.zeros(self.num_envs, self.num_bodies_robot, 3, dtype=torch.float, device=self.device)
    ...
    def _reset_idx(self, env_ids):
        ...
        self.last_root_vel[env_ids] = self.root_states_robot[env_ids, 7:13]
    ...
    def step(self, actions):
        ...
        reward_ball = self._reward_ball_chasing()
        # print("now ball chasing reward:", reward_ball)
    ...
    def _get_relative_ball_position(self):
        robot_pos = self.root_states_robot[:,0:3]  # x, y, z
        ball_pos = self.root_states_ball[:,0:3]
...
# ------------ reward functions----------------
    ...
    def _reward_ball_chasing(self):
        robot_pos = self.root_states_robot[:,0:2]  # x, y
        ball_pos = self.root_states_ball[:,0:2] # x,y

        vec_to_ball = ball_pos - robot_pos
        distance = torch.norm(vec_to_ball, dim=-1)

        reward_dist = 1.0 / (1.0 + distance)
        return reward_dist